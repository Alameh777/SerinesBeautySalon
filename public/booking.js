document.addEventListener('DOMContentLoaded', function() {
    ...
    const services = JSON.parse(table.dataset.services);
    const employees = JSON.parse(table.dataset.employees);
    ...
    document.getElementById('add_service').addEventListener('click', () => {
        ...
        newRow.querySelector('.service_select').name = `services[${counter}][service_id]`;
        newRow.querySelector('.employee_select').name = `services[${counter}][employee_id]`;
        tbody.appendChild(newRow);

        updateEmployeeOptions(newRow);
