...
<div class="card" style="padding: 24px; max-width: 600px; margin: auto; border-radius:12px; box-shadow:0 6px 18px rgba(0,0,0,0.08); background:white;">
    <h1 style="margin-bottom: 16px; font-weight:600; color:var(--gray-700);">Edit Client</h1>
    ...
    <form action="{{ route('clients.update', $client->id) }}" method="POST">
        ...
        <div style="margin-bottom:14px;">
            ...
            <label style="margin-right:12px;">
                <input type="radio" name="gender" value="male" {{ old('gender', $client->gender) == 'male' ? 'checked' : '' }}> Male
            </label>
            <label>
                <input type="radio" name="gender" value="female" {{ old('gender', $client->gender) == 'female' ? 'checked' : '' }}> Female
            </label>
        ...
        <div style="margin-bottom:20px;">
            ...
            <textarea name="notes" style="width:100%; padding:10px 12px; border-radius:8px; border:1px solid #ccc;" rows="4">{{ old('notes', $client->notes) }}</textarea>
